...
import 'Oneway_Journey_countryModelVC.dart';
...
class Oneway_ArriavalCountryVC extends StatefulWidget {
...
class _HomePageState extends State<Oneway_ArriavalCountryVC> {
  List<Country_arrival_Details> _searchResult = [];
  List<Country_arrival_Details> _countryDetails = [];
  // final List<String> countrylist = <String>['Most Popular countries...', 'India'];

  TextEditingController oneway_trpcontroller = new TextEditingController();
  ...
  Future<dynamic> OriginlocationAPI() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        for (Map user in jsonData) {
          _countryDetails.add(Country_arrival_Details.fromJson(user as Map<String, dynamic>));
      ...
      itemBuilder: (context, index) {
        ...
        return new Card(
          child: new ListTile(
              ...
              onTap: () async{
                print('filter....');
                print(oneway_trpcontroller.text = _searchResult[index].Country_Name + '  - ' + _searchResult[index].Country_code);
                oneway_trpcontroller.text = _searchResult[index].Country_Name + '  - ' + _searchResult[index].Country_code;
                Country_code = _searchResult[index].Country_code;
                Country_name = _searchResult[index].Country_Name;
                Navigator.push(
                  context,
                ...
                SharedPreferences prefs = await SharedPreferences.getInstance();
                prefs.setString("Oneway_arriavalcitykey", oneway_trpcontroller.text);
                prefs.setString("Oneway_arrivalcountry_codekey", Country_code);
                prefs.setString("Oneway_arrivalCountry_namekey", Country_name);
                prefs.setInt('Onewaytripindexkey', selectedIndex);
  ...
  Widget _buildBody() {
    return new Column(
      children: <Widget>[
        ...
        new Expanded(
            child: _searchResult.length != 0 || oneway_trpcontroller.text.isNotEmpty
                ? _buildSearchResults()
                : _buildUsersList()),
      ],
    );
    ...
    return new Padding(
      ...
      child: new Card(
        child: new ListTile(
          ...
          title: new TextField(
            controller: oneway_trpcontroller,
